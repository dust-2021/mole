...
static constexpr ULONGLONG tp_100ns_from_1601_1970 = 116444736000000000ULL;
...
void initial()
{
    auto dll_dir = L"./src/wireguard.dll";
    if (GetEnvironmentVariableW(L"NODE_ENV", nullptr, 0) != 0) {
        dll_dir = L"./wireguard.dll";
    }
    wg =
        LoadLibraryExW(dll_dir, nullptr,
...
class wireguard_adapter : public std::enable_shared_from_this<wireguard_adapter> 
{
    ...
    size_t generate_config()
    {
        ...
        for (auto &[peer_name, val] : allowed_ips)
        {
            total_size += allowed_ip_size * val.size();
        }
...
extern "C"
{
    ...
    EXPORT response create_adapter(const wchar_t *name, const u_char *public_key, const u_char *private_key, uint16_t port)
    {
        ...
        {
            return {2, "create adapter failed"};
        }
        return {0, "success"};
    }
    ...
    EXPORT response add_peer(const wchar_t *adapter_name, const wchar_t *peer_name, const char *ip, const uint16_t port, const u_char *public_key)
    {
        ...
        if (auto &handle = WireGuardHandle::getInstance(); !handle.add_peer(adapter_name, peer_name, public_key, ip, port))
        {
            return {2, "add peer failed"};
        }
        return {0, "success"};
    }
}
...
namespace test
{
    void __stdcall test_log(const WIREGUARD_LOGGER_LEVEL level, int64_t dt, const wchar_t *msg)
    {
        ...
        if (dt == 0) {
            ...
            timestamp = (dt - tp_100ns_from_1601_1970) / 10000;
...
int main()
{
    ...
    const u_char peer_key[] = {
        113, 54, 183, 51, 253, 208, 0, 141, 85, 73, 153, 40, 209, 110, 24, 169, 158, 172, 204, 231, 13, 52, 53, 46, 53,
        186, 9, 64, 182, 167, 28, 130};
    if (const auto ok = handle.add_peer(L"test", L"peer1", peer_key, "192.168.0.100", 8767); !ok)
    {
        return 0;
    }
